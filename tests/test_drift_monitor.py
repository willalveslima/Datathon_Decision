...
from src.drift_monitor_service.main import (
    app,
    PredictionLog,
    DriftMetric,
    calculate_drift_metrics as actual_calculate_drift_metrics,
)
...
TEST_PREDICTION_LOG_ENTRY = {
    ...
    "actual_outcome": None,
    "input_features_applicant": json.dumps(
        {"objetivo_profissional": "Dev Python", "local": "São Paulo"}
    ),
...
TEST_PREDICTION_LOG_ENTRY_HIRED = {
    ...
    "actual_outcome": True,
    "input_features_applicant": json.dumps(
        {"objetivo_profissional": "Dev Java", "local": "Rio de Janeiro"}
    ),
    "input_features_job": json.dumps(
        {
...
TEST_PREDICTION_LOG_ENTRY_NOT_HIRED = {
    ...
    "input_features_applicant": json.dumps(
        {"objetivo_profissional": "Analista", "local": "Belo Horizonte"}
    ),
    "input_features_job": json.dumps(
        {
            "nivel_ingles": "Básico",
...
def test_client_and_mocks():
    ...
    with patch.dict(
        ...
        with (
            patch("src.drift_monitor_service.main.create_engine") as mock_create_engine,
            ...
            mock_db_session_class.return_value = (
                mock_db_session_instance  # Quando a classe de sessão é instanciada
            )
            ...
            mock_db_session_instance.query.return_value.filter.return_value.order_by.return_value.first.return_value = (
                None
            )
...
def test_receive_feedback_success(test_client_and_mocks):
    ...
    assert (
        response.json()["message"] == "Feedback recebido e log de predição atualizado com sucesso."
    )
...
def test_receive_feedback_db_error(test_client_and_mocks):
    ...
    assert response.json()["detail"] == "Erro interno inesperado ao processar feedback."
...
def test_calculate_drift_metrics(mock_datetime, mock_joblib_load, mock_session_local):
    ...
    mock_baseline_log1.input_features_applicant = json.dumps(
        {"objetivo_profissional": "Python Dev", "local": "São Paulo"}
    )
    mock_baseline_log1.input_features_job = json.dumps(
        {"nivel_ingles": "Avançado", "nivel_profissional": "Sênior", "pais": "Brasil"}
    )
    ...
    mock_baseline_log2.input_features_applicant = json.dumps(
        {"objetivo_profissional": "Java Dev", "local": "Rio de Janeiro"}
    )
    mock_baseline_log2.input_features_job = json.dumps(
        {"nivel_ingles": "Básico", "nivel_profissional": "Júnior", "pais": "Brasil"}
    )
    ...
    mock_recent_log1.input_features_applicant = json.dumps(
        {"objetivo_profissional": "Python Dev", "local": "São Paulo"}
    )
    mock_recent_log1.input_features_job = json.dumps(
        {"nivel_ingles": "Avançado", "nivel_profissional": "Sênior", "pais": "Brasil"}
    )
    ...
    mock_recent_log2.input_features_applicant = json.dumps(
        {"objetivo_profissional": "Node.js Dev", "local": "Rio de Janeiro"}
    )
    mock_recent_log2.input_features_job = json.dumps(
        {"nivel_ingles": "Intermediário", "nivel_profissional": "Pleno", "pais": "Brasil"}
    )
