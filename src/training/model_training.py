...
try:
    ...
    print(
        "Erro: 'data/processed/merged_data_processed.csv' não encontrado. Por favor, execute as etapas anteriores para criá-lo."
    )
    exit()
...
def extract_skills_from_text(text, skills_list):
    found_skills = []
    # Garante que o texto seja string e em minúsculas
...
print(
    f"Features de habilidades criadas. Total de {len(common_skills) * 2 + 1} novas features."
)
...
# Converte probabilidades para classes binárias (threshold 0.7)
...
try:
    ...
    joblib.dump(
        tfidf_vectorizer_applicant, "../../models/tfidf_vectorizer_applicant.pkl"
    )
    ...
    joblib.dump(
        encoded_features.columns.tolist(), "../../models/encoded_feature_names.pkl"
    )
    # Salva a lista de habilidades para uso na inferência
    joblib.dump(common_skills, "../../models/common_skills.pkl")

    print(
        "Modelo Logistic Regression, vetorizadores TF-IDF, informações de colunas categóricas e habilidades comuns salvos com sucesso."
    )
...
print(
    "2. Selecionar uma vaga (por exemplo, uma nova vaga ou uma vaga existente para a qual você quer encontrar candidatos)."
)
print(
    "3. Para cada candidato disponível (que ainda não foi contratado para essa vaga):"
)
print(
    "   a. Criar uma entrada de dados combinando as características do candidato "
    "e as características da vaga "
    "(similar à forma como o 'X' foi construído)."
)
print(
    "   b. Pré-processar esta entrada usando os *mesmos* vetorizadores TF-IDF, a lógica de codificação categórica E a extração de habilidades usados no treinamento."
)
print(
    "   c. Usar o modelo `model.predict_proba()` para obter a probabilidade de contratação."
)
print(
    "4. Classificar os candidatos com base nessas probabilidades para gerar uma lista de recomendação."
)
